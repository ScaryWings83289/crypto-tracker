...
const SaveButton = ({ data }: { data: CoinsDataType }) => {
  const storageContextData = useContext(StorageContext);
  const savedCoins = storageContextData?.savedCoins as string[];
  const handleSavedCoin = storageContextData?.handleSavedCoin as (
    coinId: string
  ) => void;
  const handleRemoveCoin = storageContextData?.handleRemoveCoin as (
    coinId: string
  ) => void;
  ...
  const handleClick = (e: MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    handleSavedCoin(data.id);

    if (savedCoins.includes(data.id)) {
      handleRemoveCoin(data.id);
    } else {
      handleSavedCoin(data.id);
    }
  };
    ...
    >
      <svg
        className={`w-[1.5rem] ml-1.5 ${
          savedCoins.includes(data.id) ? "fill-cyan" : "fill-gray-100"
        }hover:fill-cyan`}
        width="30"
        height="30"
        viewBox="0 0 30 30"
...
const Saved = () => {
  ...
  const savedData = storageContextData?.savedData as CoinsDataType[];
  const handleSaveReset = storageContextData?.handleSaveReset as () => void;
            ...
            <tbody>
              {savedData.map((coin: CoinsDataType) => (
                ...
                >
                  ...
                  <td
                    className={
                      coin.market_cap_change_percentage_24h < 0
                        ? "py-4 sm:table-cell hidden text-red"
                        : "py-4 sm:table-cell hidden text-green"
                    }
                  >
                  ...
                  <td
                    className={
                      (coin.price_change_percentage_24h_in_currency as number) <
                      0
                        ? "py-4 sm:table-cell hidden text-red"
                        : "py-4 sm:table-cell hidden text-green"
                    }
                  ...
                  <td
                    className={
                      (coin.price_change_percentage_7d_in_currency as number) <
                      0
                        ? "py-4 sm:table-cell hidden text-red"
                        : "py-4 sm:table-cell hidden text-green"
                    }
